...
/* ============================================================
   ...
   ============================================================ */
const DEFAULT_HINT = 'Click, tap, or drag to shoot Cupid’s arrow ♡';
...
function metrics(){
  ...
  const sw = s.width || aimStage.offsetWidth || 400;
  const sh = s.height || aimStage.offsetHeight || 500;
  const sleft = s.left || 0;
  const stop = s.top || 0;

  // Exact geometric defaults based on CSS layout (.archer bottom 12%, .target top 6%)
  let nx = sw / 2;
  let ny = sh - (sh * 0.12) - 72;
  let tx = sw / 2;
  let ty = (sh * 0.06) + 71;

  // Validate live DOM measurements if available
  const n = nockEl.getBoundingClientRect();
  const e = targetEnv.getBoundingClientRect();

  if (n && n.width > 0 && n.left > 0) {
    const liveNx = n.left + n.width / 2 - sleft;
    const liveNy = n.top + n.height / 2 - stop;
    if (liveNx > 40 && liveNx < sw - 40 && liveNy > sh * 0.4 && liveNy < sh) {
      nx = liveNx;
      ny = liveNy;
    }
  }
  if (e && e.width > 0 && e.left > 0) {
    const liveTx = e.left + e.width / 2 - sleft;
    const liveTy = e.top + e.height / 2 - stop;
    if (liveTx > 40 && liveTx < sw - 40 && liveTy > 10 && liveTy < sh * 0.5) {
      tx = liveTx;
      ty = liveTy;
    }
  }
  ...
  return {
    w: sw, h: sh, left: sleft, top: stop,
    nx, ny, tx, ty,
    thw: (e && e.width ? e.width / 2 : 100),
    thh: (e && e.height ? e.height / 2 : 71),
  };
}
...
function renderAim(m){
  ...
  const offset = -drawn;
  arrow.style.transform = `rotate(${deg}deg) translateX(${offset}px)`;
...
function fire(power){
  ...
  if (power == null) {
    const rawPower = clamp01((drawn - MIN_DRAW) / (MAX_DRAW - MIN_DRAW));
    power = rawPower < 0.2 ? 1.0 : rawPower;
  }
  flying = true;
  aiming = false;
  ...
  let d = -launchDraw;
  const targetDist = Math.hypot(m.tx - m.nx, m.ty - m.ny) - 35;
  ...
  function step(now){
    ...
    arrow.style.transform = `rotate(${dir * 180 / Math.PI}deg) translateX(${d}px)`;
    arrow.classList.add('is-on');

    const cos = Math.cos(dir), sin = Math.sin(dir);
    const tipx = m.nx + d * cos;
    const tipy = m.ny + d * sin;

    // Precise envelope hit detection: letter ONLY opens if the arrow hits the envelope
    const hitX = Math.abs(tipx - m.tx) <= m.thw + 18;
    const hitY = Math.abs(tipy - m.ty) <= m.thh + 18;
    ...
    // Out of bounds miss: reset without opening letter
    if (tipy < -60 || tipx < -60 || tipx > m.w + 60 || tipy > m.h + 60){
      onMiss();
      return;
    }
...
function onHit(m, dir){
  ...
  flying = false; drawn = 0; aiming = false;
  const a = (dir == null) ? angle : dir;
  const deg = a * 180 / Math.PI;
  const targetDist = Math.hypot(m.tx - m.nx, m.ty - m.ny) - 35;
  arrow.style.transform = `rotate(${deg}deg) translateX(${targetDist}px)`;
...
// ---- input: pointer & click — window-wide aim on move, draw on down, fire on release or click ----
function isWindowOpen() {
  return win.classList.contains('is-open');
}

window.addEventListener('pointermove', (e) => {
  if (!flying && !isWindowOpen()) updateAim(e.clientX, e.clientY);
});

window.addEventListener('pointerdown', (e) => {
  if (flying || isWindowOpen()) return;
  if (e.target.closest('#brand, .window, .winbtn')) return;
  const m = metrics();
...
// Window-wide click trigger: any click or tap anywhere on screen fires the arrow instantly!
...
function reset(){
  catState = 'idle';
  title.textContent = 'Happy 1 year, my love ♡ Still love me?';
